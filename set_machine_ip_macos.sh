...
source "$SCRIPT_DIR/config/set_machine_ip_macos.conf"
...
usage() { cat <<EOF
set_machine_ip_macos.sh - updates config files using directories from config/set_machine_ip_macos.conf
...
Edit directories in: $SCRIPT_DIR/config/set_machine_ip_macos.conf
Optionally set NEW_IP in the config to skip interactive selection.
EOF
}

parse_args() { while [[ $# -gt 0 ]]; do case "$1" in -n|--dry-run) DRY_RUN=true; shift;; -y|--yes) ASSUME_YES=true; shift;; --no-color) NO_COLOR=1; shift;; -h|--help) usage; exit 0;; *) ui_error "Unknown option: $1"; usage; exit 1;; esac; done }
